...
IF EXIST %MINICONDAPATH% (
	IF EXIST %MINIENVPATH% (
		ECHO Attempting to activate conda environment %FERMO_VER% & CALL %MINICONDAPATH% & conda activate %FERMO_VER% & ECHO %FERMO_VER% successfully activated. & python src\fermo\app.py & PAUSE & GOTO :exit
	)
)
