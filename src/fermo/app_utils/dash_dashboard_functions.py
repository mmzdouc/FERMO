...
def generate_subsets(
    ...
    filt_df = samples[sample].loc[((samples[sample][
        'rel_intensity_score'
    ] >= thresholds['rel_intensity_threshold'][0]) & (samples[sample][
        'rel_intensity_score'
    ] <= thresholds['rel_intensity_threshold'][1])) & ((samples[sample][
        'novelty_score'
    ] >= thresholds['novelty_threshold'][0]) & (samples[sample][
        'novelty_score'
    ] <= thresholds['novelty_threshold'][1])) & ((samples[sample][
        'convolutedness_score'
    ] >= thresholds['peak_overlap_threshold'][0]) & (samples[sample][
        'convolutedness_score'
    ...
    if thresholds['filter_adduct_isotopes'] != '':
        temp_set = set()
        for feature in filtered_thrsh_set:
            if filter_str_regex(thresholds[
    ...
    if thresholds['filter_annotation'] != '':
        ...
        for feature in filtered_thrsh_set:
            ...
            if feature_dicts[str(feature)]['ms2query']:
                search_list.append(feature_dicts[str(feature)][
                    'ms2query_results'
                ][0]['analog_compound_name'])
            ...
            if filter_str_regex(thresholds['filter_annotation'],','.join(
                [i for i in search_list]
            )):
                temp_set.add(feature)
        filtered_thrsh_set = filtered_thrsh_set.intersection(temp_set)
    ...
    if (thresholds['filter_precursor_min'] != '' or (
        ...
        if (mz_min is not None) and (mz_max is not None):
            ...
            for feature in filtered_thrsh_set:
                if ((mz_min <= feature_dicts[str(feature)][
                    'precursor_mz'
                ] <= mz_max)):
                    temp_set.add(feature)
            filtered_thrsh_set = filtered_thrsh_set.intersection(temp_set)
    ...
    if thresholds['filter_group'] != '':
        temp_set = set()
        for feature in filtered_thrsh_set:
            if filter_str_regex(thresholds['filter_group'],','.join(
                [i for i in feature_dicts[str(feature)]['set_groups']]
            ),):
                temp_set.add(feature)
        filtered_thrsh_set = filtered_thrsh_set.intersection(temp_set)
    ...
    if thresholds['filter_group_cliques'] != '':
        ...
        for feature in filtered_thrsh_set:
            try:
                if filter_str_regex(
                    thresholds['filter_group_cliques'], ','.join(
                        [i for i in feature_dicts[str(feature)][
                            'set_groups_clique'
                        ]]
                    ),
                ):
    ...
    if (thresholds[
        ...
        if (snum_min is not None) and (snum_max is not None):
            ...
            for feature in filtered_thrsh_set:
                if ((snum_min <= len(feature_dicts[str(feature)][
                    'presence_samples'
                ]) <= snum_max)):
                    temp_set.add(feature)
            filtered_thrsh_set = filtered_thrsh_set.intersection(temp_set)
            ...
            temp_set = set()
            for feature in filtered_thrsh_set:
                if len(feature_dicts[str(feature)][
                    'presence_samples'
                ]) <= snum_max:
                    temp_set.add(feature)
            filtered_thrsh_set = filtered_thrsh_set.intersection(temp_set)
            ...
            if feature_dicts[str(feature)]['dict_fold_diff']:
                for i in feature_dicts[str(feature)]['dict_fold_diff']:
                    if ((feature_dicts[str(feature)][
                        ...
                        'filter_fold_greater_int'
                    ])) and (filter_str_regex(thresholds[
                        'filter_fold_greater_regex'
                    ],i,))):
                        temp_set.add(feature)
        filtered_thrsh_set = filtered_thrsh_set.intersection(temp_set)
    ...
    if ((thresholds[
        ...
        'filter_fold_greater_exclude_int'
    ] != '') and (thresholds['filter_fold_greater_exclude_regex'] != '')):
        temp_set = set()
        for feature in filtered_thrsh_set:
            if feature_dicts[str(feature)]['dict_fold_diff']:
                for i in feature_dicts[str(feature)]['dict_fold_diff']:
    ...
    for ID in all_select_no_blank.difference(select_sample_spec):
        if ((len(feature_dicts[str(ID)][
            'set_groups'
        ]) == 1) and not ('GENERAL' in feature_dicts[str(ID)]['set_groups'])):
...
def calc_sample_unique_cliques(
    ...
    for sample in samples:
        ...
        for ID in samples_subsets[sample]['all_nonblank']:
            if (
                (len(feature_dicts[str(ID)][
                    'set_groups_clique'
                ]) == 1) and (sample in feature_dicts[str(ID)][
                    'presence_samples'
                ])
            ):
                unique_cliques.add(
...
def prepare_sample_scores_df(
    ...
    sample_scores = pd.DataFrame({
        ...
        'Selected networks': [
            sample_sel_cliques[i] for i in sample_sel_cliques
        ],
        'Diversity score': diversity_score,
        'Spec score': specificity_score,
        'Mean Novelty score': [
            sample_mean_novelty[i] for i in sample_mean_novelty
        ],
        'Total': [len(samples_subsets[i]['all_features']) for i in samples],
        'Non-blank': [len(samples_subsets[i][
            'all_nonblank'
        ]) for i in samples],
...
def calc_diversity_score(
    ...
    for sample in samples:
        try:
            list_div_scores.append(round((len(set(sample_stats[
                "cliques_per_sample"
            ][sample]).difference(set(sample_stats[
                "set_blank_cliques"
            ]))) / len(set(sample_stats[
                "set_all_cliques"
            ]).difference(set(sample_stats["set_blank_cliques"])))),2))
        except:
    ...
    for sample in samples:
        try:
            list_spec_scores.append(round(
                (len(sample_unique_cliques[sample])) / len(set(sample_stats[
                    "cliques_per_sample"
                ][sample]).difference(set(sample_stats[
                    "set_blank_cliques"
                ]))), 2
...
def append_scatter_text(
    ...
    cosine_ann = (cosine_ann[:75] + '...') if len(
        cosine_ann
    ) > 75 else cosine_ann
    ...
    ms2query_ann = (ms2query_ann[:75] + '...') if len(
        ms2query_ann
    ) > 75 else ms2query_ann

    text = (
        f'''Feature ID <b>{row["feature_ID"]}</b><br><i>m/z</i><b>{
            row["precursor_mz"]
        }</b><br>RT (min) <b>{row["retention_time"]}</b><br>Lib. annot. <b>{
            cosine_ann
        }</b><br>MS2Query <b>{ms2query_ann}</b><br>'''
    )
    return go.Scatter(
...
def plot_central_chrom(
    ...
    for id, row in samples[sel_sample].iterrows():
        if (int(row['feature_ID']) in subsets[sel_sample][
            'blank_ms1'
        ] and sel_all_vis == 'ALL'):
            fig.add_trace(
                append_scatter_text(
        ...
        elif (int(row['feature_ID']) in subsets[sel_sample][
            'blank_ms1'
    ...
    if isinstance(active_feature_index, int):
        if feature_dicts[str(active_feature_id)]['similarity_clique']:
            for clique_member in (
                sample_stats['cliques'][str(feature_dicts[
                    str(active_feature_id)
                ]['similarity_clique_number'])][0]
            ):
                if clique_member != active_feature_id:
                    try:
                        row = samples[selected_sample].loc[samples[
                            selected_sample
                        ]['feature_ID'] == clique_member].squeeze()
                        fig.add_trace(
                            append_scatter_text(
                                row,
        ...
        fig.add_trace(
            append_scatter_text(
                samples[selected_sample].loc[
                    samples[selected_sample]['feature_ID'] == active_feature_id
                ].squeeze(),
...
def plot_sample_chrom(
    ...
    for i in range(len(feature_dicts[str(ID)]['presence_samples'])):
        ...
        for id, row in samples[sample].iterrows():
            ...
            elif ((row['pseudo_chrom_trace'][0][0] >= rt_min) and (
...
def modify_feature_info_df(
    ...
    if feat_dicts[ID]['ms2query']:
        ...
        superclass_ms2query = ''.join([
            str(feat_dicts[ID]['ms2query_results'][0][
    ...
    if feat_dicts[ID]['similarity_clique']:
        sim_clique_len = len(
            sample_stats['cliques'][
                str(feat_dicts[ID]['similarity_clique_number'])
            ][0]
        )
    ...
    data = [
        ...
        ['Feature intensity (relative)', round(samples[smpl].at[
            index, 'rel_intensity_score'
        ],2)],
        ...
        ['Peak overlap (%)', round((samples[smpl].at[
            index, 'convolutedness_score'
        ] * 100),0)],
        [placeholder, placeholder],
        ['Spectral library: best match', cosine_annotation],
        ['**[MS2Query](https://github.com/iomega/ms2query)**: \
        best match/analog', ann_ms2query],
        ['**[MS2Query](https://github.com/iomega/ms2query)**:\
         <i>m/z</i> difference to best match/analog', mass_diff_ms2query],
        ['**[MS2Query](https://github.com/iomega/ms2query)**: \
        predicted class of best match/analog', class_ms2query],
        ['**[MS2Query](https://github.com/iomega/ms2query)**: \
        predicted superclass of best match/analog', superclass_ms2query],
        [placeholder, placeholder],
        ['Feature found in groups\
        ', ("".join(f"{i}<br>" for i in feat_dicts[ID]['set_groups']))],
        ['Fold-differences across groups\
        ', ("".join(str(i) for i in fold_diff_list))],
        ['Intensity per sample (highest to lowest)\
        ', ("".join(str(i) for i in combined_list_int))],
        ['QuantData per sample (highest to lowest)\
        ', ("".join(str(i) for i in combined_list_bio))],
        ['Original QuantData (highest to lowest)\
        ', ("".join(str(i) for i in combined_list_orig_bio))],
        ['Putative adducts', ("".join(str(i) for i in combined_list_adducts))],
        [placeholder, placeholder],
        ['Spectral similarity network ID\
        ', feat_dicts[ID]['similarity_clique_number']],
...
def empty_feature_info_df():
    ...
    data = [
        ...
        ['**[MS2Query](https://github.com/iomega/ms2query)**: \
        best match/analog', None],
        ['**[MS2Query](https://github.com/iomega/ms2query)**: \
        <i>m/z</i> difference to best match/analog', None],
        ['**[MS2Query](https://github.com/iomega/ms2query)**: \
        predicted class of best match/analog', None],
        ['**[MS2Query](https://github.com/iomega/ms2query)**: \
        predicted superclass of best match/analog', None],
        [placeholder, placeholder],
...
def generate_cyto_elements(
    ...
    if feat_dicts[ID]['similarity_clique']:
        ...
        precursor_list = [feat_dicts[
            str(i)
        ]['precursor_mz'] for i in node_list]
        id_precursor_dict = {
            node_list[i]: [
                precursor_list[i], feat_dicts[str(node_list[i])]['feature_ID'],
        ...
        if len(node_list) <= 250:
            ...
            nodes = [
                ...
                if ((id_precursor_dict[i][1] in sample_stats[
                    'features_per_sample'
                ][sel_sample]) and (sel_sample in feat_dicts[
                    str(i)
                ]['presence_samples']) and (len(feat_dicts[
                    str(i)
                ]['presence_samples']) == 1)
                )
                ...
                if ((id_precursor_dict[i][1] in sample_stats[
                    ...
                    str(i)
                ]['set_groups']) == 1) and not ('GENERAL' in feat_dicts[
                    str(i)
                ]['set_groups'])
                )
                ...
                if (
                    (
                        id_precursor_dict[i][1] in sample_stats[
                            'features_per_sample'
                        ][sel_sample]
                    )
                )
                ...
                if (
                    (len(feat_dicts[str(i)]['set_groups']) == 1) and (
                        feat_dicts[str(i)][
                            'set_groups'
...
def add_nodedata(
    ...
    annotation = ''.join([
        ...
        '<b>(user-library)</b>, <br>',
        (feat_dicts[str(data['id'])]['ms2query_results'][0][
    ...
    superclass_ms2query = ''.join([
        (str(feat_dicts[str(data['id'])]['ms2query_results'][0][
            'npc_superclass_results'
        ] if feat_dicts[str(data['id'])]['ms2query'] else "None ")),
        '<b>(NPC superclass)</b>, <br>',
        (str(feat_dicts[str(data['id'])][
            'ms2query_results'
        ][0]['cf_superclass'] if feat_dicts[
            str(data['id'])
        ]['ms2query'] else "None ")),
        '<b>(CF superclass)</b>',
    ])
    ...
    content = [
        ...
        ['Retention time (avg)', feat_dicts[str(data['id'])][
            'average_retention_time'
        ]],
...
def feat2table_library(
    ...
    if feat_dicts[str(row['feature_ID'])]['cosine_annotation']:
        return ''.join([
            str(feat_dicts[str(row['feature_ID'])][
                'cosine_annotation_list'
            ][0]['name']),
            '(score: ',
            str(round(feat_dicts[str(row['feature_ID'])][
                'cosine_annotation_list'
            ][0]['score'],2)),
            ')',
        ])
...
def export_sel_peaktable(
    ...
    df['QuantData_associated'] = df.apply(
        lambda row: feat2table(
            feat_dicts, row, 'bioactivity_associated'
        ), axis=1,
    )
...
def prepare_cosine_info(
    ...
    if feature_dicts[ID]['cosine_annotation']:
        return "".join([
            ...
            ' (score: ',
            str(round(feature_dicts[ID]['cosine_annotation_list'][0][
                'score'
            ], 2)),
            ')',
        ])
